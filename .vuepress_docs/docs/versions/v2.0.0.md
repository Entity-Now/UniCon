...
| `Notification/NotificationDispatcher.cs` | <code>Channel&amp;lt;T&amp;gt;</code> 异步分发器，独立线程消费，Callback 异常隔离 |
...
### UniconSubscription
- <code>Callback</code> 从 <code>Action&amp;lt;DataValue&amp;lt;object&amp;gt;&amp;gt;</code> 改为 <code>Func&amp;lt;DataValue&amp;lt;object&amp;gt;, Task&amp;gt;</code>
- 新增 `MaxQueueLength`、`OverflowPolicy`、`TagMetadata` 字段
- 移除 `LastPolledTime`（迁移至 `ScanGroup`）

### <code>DataValue&amp;lt;T&amp;gt;</code>
- 新增 `ServerTimestamp`（网关到达时间戳）
- 新增 `QualityCode Quality`（OPC UA 兼容质量码）

### IUniconDriver
- <code>SubscribeAsync</code> callback 升级为 <code>Func&amp;lt;DataValue&amp;lt;object&amp;gt;, Task&amp;gt;</code>
- 新增 `StateChanged` 事件
...
| `sub.Callback(value)` 同步调用 | <code>Channel&amp;lt;T&amp;gt;</code> 投递，独立线程异步消费 |
